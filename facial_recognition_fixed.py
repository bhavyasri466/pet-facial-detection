...
def decode_petimage(jpg_as_text):
    """The working image decoding function from your old code"""
    try:
        jpg_original = base64.b64decode(jpg_as_text)
        jpg_as_np = np.frombuffer(jpg_original, dtype=np.uint8)
        image_buffer = cv2.imdecode(jpg_as_np, flags=1)
        if image_buffer is not None:
            return cv2.cvtColor(image_buffer, cv2.COLOR_BGR2RGB)
        return None
    except Exception as e:
        logger.error(f"Error in decode_petimage: {str(e)}")
        return None

def retriving_petimage(image_id):
    """Revised image retrieval using the proven approach from your old code"""
    try:
        ...
        try:
            # Directly use the approach that worked in your old code
            grid_file = fs.get(image_id)
            img_text = grid_file.read()
            
            # First try the original working method
            pet_img = decode_petimage(img_text)
            ...
            try:
                # Try OpenCV directly
                nparr = np.frombuffer(img_text, np.uint8)
                img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
                if img is not None:
                    return cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            except Exception as e:
                logger.debug(f"OpenCV direct decode failed: {str(e)}")
                
            try:
                # Try PIL as fallback
                img = Image.open(io.BytesIO(img_text))
                return cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
            except Exception as e:
                logger.debug(f"PIL decode failed: {str(e)}")
                
            # Debugging: Save the problematic image
            debug_filename = f"failed_image_{image_id}.bin"
            with open(debug_filename, 'wb') as f:
                f.write(img_text)
            logger.error(f"All decoding methods failed for image {image_id}. Raw data saved to {debug_filename}")
...
def get_frames_to_process():
    ...
    try:
        ...
        logger.info(f"Found {len(frames)} valid frames (skipped {skipped_missing} with missing images)")
        return frames
    except Exception as e:
