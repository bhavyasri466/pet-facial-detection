...
class WrongTagFacialProcessor:
    ...
    def load_all_pickles_from_folder(self):
        if not os.path.exists(self.pickle_folder):
            self.logger.warning(f"Pickle folder '{self.pickle_folder}' does not exi st.")
            return
    ...
    def process_wrong_tag_frames_parallel(self, max_workers=os.cpu_count()):
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                if result:
                    self.logger.info(f"✅ Frame {result} processed.")
