...
def verify_image_exists(image_id):
    """Check if image exists and is accessible in GridFS"""
    try:
        ...
        # Try to actually read the file to verify it's not corrupted
        grid_file = fs.get(image_id)
        try:
            img_data = grid_file.read()
            # Quick test if the image is readable
            Image.open(io.BytesIO(img_data)).verify()
            return True
        except Exception as e:
            logger.error(f"Image {image_id} exists but is corrupted: {str(e)}")
            return False
    except Exception as e:
...
def fix_corrupted_jpeg(img_data):
    ...
    try:
        ...
        if jpeg_start != -1 and jpeg_end != -1:
            return img_data[jpeg_start:jpeg_end+2]
        return img_data
    except:
        return img_data

def retriving_petimage(image_id):
    """Enhanced image retrieval with multiple fallback methods"""
    try:
        if not verify_image_exists(image_id):
            logger.error(f"Image {image_id} not found or corrupted in GridFS")
            return None
        ...
        decode_methods = [
            ...
            lambda: cv2.cvtColor(
                np.array(Image.open(io.BytesIO(img_data))),
                cv2.COLOR_RGB2BGR
            ),
            
            # Method 3: Try to fix corrupted JPEGs
            lambda: cv2.imdecode(
                np.frombuffer(fix_corrupted_jpeg(img_data), np.uint8),
                cv2.IMREAD_COLOR
            ),
            
            # Method 4: Try with different OpenCV flags
            lambda: cv2.imdecode(np.frombuffer(img_data, np.uint8), cv2.IMREAD_UNCHANGED),
            
            # Method 5: Try loading as PNG if JPEG fails
