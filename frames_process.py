...
# --- Load All Pickle Images with Auto-detection ---
def load_reference_images(pickle_file):
    ...
    if isinstance(data, list):
        ...
        for key, item in data.items():
            if isinstance(item, np.ndarray):
                ...
                for subkey in ['image', 'img', 'frame']:
                    if subkey in item and isinstance(item[subkey], np.ndarray):
                        images.append(item[subkey])
                        break
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "web":
        ...
        pickle_file = "images_2025-05-03_batch1.pickle"
        reference_images = load_reference_images(pickle_file)
